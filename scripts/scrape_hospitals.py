...
hospital category pull across one or more lat/long pins (with full
pagination), and merges results into data/hospitals_raw/hospitals_master.csv
...
Usage:
  ...
  python scripts/scrape_hospitals.py --pins pins.csv
  python scripts/scrape_hospitals.py --pin "27.4945,-80.3383" --distance 10
...
OUT_DIR = REPO_ROOT / "data" / "hospitals_raw"
...
# (otherwise Django bounces it with ?e=1). This pulls the RAW hospital
# universe: basic_category in {hospital} OR category_primary in
# {hospital, emergency_room, medical_center}, no excludes, no confidence gate.
CATEGORY_PARAMS = {
    "apply_saved_filter": "",
    "update_saved_filter": "",
    "apply_filter": "1",
    ...
    "basic_category_include_values": "hospital\r\n",
    "category_primary_include_values": "hospital\r\nemergency_room\r\nmedical_center\r\n",
...
def load_existing() -> dict[str, dict[str, str]]:
    csv_path = OUT_DIR / "hospitals_master.csv"
...
def write_master(records: dict[str, dict[str, str]]) -> None:
    ...
    with (OUT_DIR / "hospitals_master.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=CSV_COLUMNS, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(all_rows)
    (OUT_DIR / "hospitals_master.json").write_text(

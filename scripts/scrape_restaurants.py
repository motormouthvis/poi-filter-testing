"""Authenticated scraper for the POI admin change-list (RESTAURANTS pull).

Logs into the Django admin using credentials from a local .env, runs the
restaurant category pull across one or more lat/long pins (with full
pagination), and merges results into
data/restaurants_raw/restaurants_master.csv (deduped by Overture id).
...
Usage:
  ...
  python scripts/scrape_restaurants.py --pins data/coffee_tea_raw/pins.csv
  python scripts/scrape_restaurants.py --pin "27.4945,-80.3383" --distance 10
...
OUT_DIR = REPO_ROOT / "data" / "restaurants_raw"
...
# (otherwise Django bounces it with ?e=1). This pulls the RAW restaurant
# universe: basic_category = restaurant, no excludes, no confidence gate.
CATEGORY_PARAMS = {
    "apply_saved_filter": "",
    "update_saved_filter": "",
    "apply_filter": "1",
    ...
    "basic_category_include_values": "restaurant\r\n",
    ...
    "query_builder": "(basic_category_include)\r\n",
}
...
def load_existing() -> dict[str, dict[str, str]]:
    csv_path = OUT_DIR / "restaurants_master.csv"
...
def write_master(records: dict[str, dict[str, str]]) -> None:
    ...
    with (OUT_DIR / "restaurants_master.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=CSV_COLUMNS, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(all_rows)
    (OUT_DIR / "restaurants_master.json").write_text(

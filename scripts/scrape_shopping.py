"""Authenticated scraper for the POI admin change-list (shopping centers / malls).

Logs into the Django admin using credentials from a local .env, runs the
shopping-center/mall category pull across one or more lat/long pins (with full
pagination), and merges results into
data/shopping_centers_raw/shopping_centers_master.csv (deduped by Overture id).
...
Usage:
  ...
  python scripts/scrape_shopping.py --pins pins.csv
  python scripts/scrape_shopping.py --pin "27.4945,-80.3383" --distance 10
...
OUT_DIR = REPO_ROOT / "data" / "shopping_centers_raw"
...
# (otherwise Django bounces it with ?e=1). This pulls the RAW shopping-center /
# mall universe: basic_category in {shopping_center, shopping_mall} OR
# category_primary in {shopping_center, shopping_mall, outlet_mall, strip_mall},
# no excludes, no confidence gate.
CATEGORY_PARAMS = {
    "apply_saved_filter": "",
    "update_saved_filter": "",
    "apply_filter": "1",
...
def load_existing() -> dict[str, dict[str, str]]:
    csv_path = OUT_DIR / "shopping_centers_master.csv"
...
def write_master(records: dict[str, dict[str, str]]) -> None:
    ...
    with (OUT_DIR / "shopping_centers_master.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=CSV_COLUMNS, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(all_rows)
    (OUT_DIR / "shopping_centers_master.json").write_text(

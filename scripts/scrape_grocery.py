"""Authenticated scraper for the POI admin change-list (GROCERY pull).

Logs into the Django admin using credentials from a local .env, runs the
grocery/supermarket category pull across one or more lat/long pins (with full
pagination), and merges results into data/grocery_raw/grocery_master.csv
...
Usage:
  ...
  python scripts/scrape_grocery.py --pins pins.csv
  python scripts/scrape_grocery.py --pin "27.4945,-80.3383" --distance 10
...
OUT_DIR = REPO_ROOT / "data" / "grocery_raw"
...
# (otherwise Django bounces it with ?e=1). This pulls the RAW grocery universe:
# basic_category in {food_and_beverage_store} OR category_primary in
# {supermarket, grocery_store, specialty_grocery_store, wholesale_grocer},
# no excludes, no name whitelist, no confidence gate.
CATEGORY_PARAMS = {
    "apply_saved_filter": "",
    "update_saved_filter": "",
    "apply_filter": "1",
    ...
    "basic_category_include_values": "food_and_beverage_store\r\n",
    "category_primary_include_values": "supermarket\r\ngrocery_store\r\nspecialty_grocery_store\r\nwholesale_grocer\r\n",
...
def load_existing() -> dict[str, dict[str, str]]:
    csv_path = OUT_DIR / "grocery_master.csv"
...
def write_master(records: dict[str, dict[str, str]]) -> None:
    ...
    with (OUT_DIR / "grocery_master.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=CSV_COLUMNS, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(all_rows)
    (OUT_DIR / "grocery_master.json").write_text(

"""Authenticated scraper for the POI admin change-list (GYMS / FITNESS).

Logs into the Django admin using credentials from a local .env, runs the
gym/fitness category pull across one or more lat/long pins (with full
pagination), and merges results into data/gyms_raw/gyms_master.csv
...
Usage:
  ...
  python scripts/scrape_gyms.py --pins pins.csv
  python scripts/scrape_gyms.py --pin "27.4945,-80.3383" --distance 10
...
OUT_DIR = REPO_ROOT / "data" / "gyms_raw"
...
# (otherwise Django bounces it with ?e=1). This pulls the RAW gym/fitness
# universe: basic_category in {gym, fitness_center} OR category_primary in the
# fitness set, no excludes, no confidence gate.
CATEGORY_PARAMS = {
    "apply_saved_filter": "",
    "update_saved_filter": "",
    "apply_filter": "1",
...
def load_existing() -> dict[str, dict[str, str]]:
    csv_path = OUT_DIR / "gyms_master.csv"
...
def write_master(records: dict[str, dict[str, str]]) -> None:
    ...
    with (OUT_DIR / "gyms_master.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=CSV_COLUMNS, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(all_rows)
    (OUT_DIR / "gyms_master.json").write_text(

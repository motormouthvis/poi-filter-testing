"""Authenticated scraper for the POI admin change-list (NIGHTLIFE pull).

Logs into the Django admin using credentials from a local .env, runs the
nightlife (bars / clubs / lounges) category pull across one or more lat/long
pins (with full pagination), and merges results into
data/nightlife_raw/nightlife_master.csv (deduped by Overture id).
...
Usage:
  ...
  python scripts/scrape_nightlife.py --pins pins.csv
  python scripts/scrape_nightlife.py --pin "27.4945,-80.3383" --distance 10
...
OUT_DIR = REPO_ROOT / "data" / "nightlife_raw"
...
# (otherwise Django bounces it with ?e=1). This pulls the RAW nightlife
# universe: basic_category contains {bar, nightlife} OR category_primary
# matches one of the anchored bar/club/lounge tokens, no excludes, no
# confidence gate.
#
# NOTE on substring matching (see docs/admin-filter-builder.md): matching is
# "contains", so bare tokens pollute badly. A test pin showed bare "bar"
# pulling barber (35), barbecue_restaurant (24), bar_and_grill (10), and bare
# "pub" pulling public_and_government_association (33), notary_public, etc.
# We therefore use ENDS-WITH anchors: "*bar" matches bar / wine_bar / sports_bar
# / tiki_bar / smoothie_juice_bar but NOT barber or barbecue_restaurant; "*pub"
# matches pub / irish_pub / gastropub but NOT public_* / notary_public. This
# subsumes the individual *_bar tokens. Specific contains tokens (night_club,
# nightclub, lounge, brewery, karaoke) carry no pollution.
CATEGORY_PARAMS = {
    "apply_saved_filter": "",
    "update_saved_filter": "",
    "apply_filter": "1",
    ...
    "category_primary_include_values": (
        "*bar\r\nnight_club\r\nnightclub\r\n*pub\r\nlounge\r\nbrewery\r\nkaraoke\r\n"
    ),
...
def load_existing() -> dict[str, dict[str, str]]:
    csv_path = OUT_DIR / "nightlife_master.csv"
...
def write_master(records: dict[str, dict[str, str]]) -> None:
    ...
    with (OUT_DIR / "nightlife_master.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=CSV_COLUMNS, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(all_rows)
    (OUT_DIR / "nightlife_master.json").write_text(

"""Authenticated scraper for the POI admin change-list (PARKS).

Logs into the Django admin using credentials from a local .env, runs the
parks category pull across one or more lat/long pins (with full pagination),
and merges results into data/parks_raw/parks_master.csv (deduped by Overture id).
...
Usage:
  ...
  python scripts/scrape_parks.py --pins pins.csv
  python scripts/scrape_parks.py --pin "27.4945,-80.3383" --distance 10
...
OUT_DIR = REPO_ROOT / "data" / "parks_raw"
...
# (otherwise Django bounces it with ?e=1). This pulls the RAW parks universe:
# basic_category contains `park` OR category_primary in the parks set, no
# excludes, no confidence gate.
CATEGORY_PARAMS = {
    "apply_saved_filter": "",
    "update_saved_filter": "",
    "apply_filter": "1",
...
def load_existing() -> dict[str, dict[str, str]]:
    csv_path = OUT_DIR / "parks_master.csv"
...
def write_master(records: dict[str, dict[str, str]]) -> None:
    ...
    with (OUT_DIR / "parks_master.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=CSV_COLUMNS, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(all_rows)
    (OUT_DIR / "parks_master.json").write_text(
